     ...
     <div class="row">
        <div class="col-md-6">
            ...
            <div class="card-header">
                <h3 class="card-title">Edición de Cátedras</h3>
            </div>
            <!-- /.card-header -->
            <div class="card-body">
                <form action=" {{ route('catedras.update',['catedra' => $catedra]) }} " method="POST">
                    {{ csrf_field() }}
                    {{ method_field('PUT') }}
                            ...
                            <div class="form-group">
                                ...
                                <select class="custom-select mb-3 {{ $errors->has("curso_id")?"is-invalid":'' }}" name="curso_id">
                                    @foreach($cursos as $curso)
                                        <option value="{{ $curso->id }}" @if($curso->id==$catedra->curso_id) selected @endif>{{ ucwords($curso->nombre) }}</option>
                                    @endforeach
                                </select>
                            </div>
                        </div>
                    ...
                    <div class="row">
                        <div class="col-md-6">
                            <div class="form-group">
                                ...
                                <input type="text"
                                @if($catedra->presentes()->count()>0)
                                    disabled
                                @endif
                                class="form-control mb-3 {{ $errors->has("fecha_inicio")?"is-invalid":'' }}" name="fecha_inicio" value="{{ old('fecha_inicio',$catedra->fecha_inicio()) }}" autocomplete="off" >
                            </div>
                        </div>
                        <div class="col-md-6">
                            <div class="form-group">
                                ...
                                @if($catedra->presentes()->count()>0)
                                    disabled
                                @endif
                                class="form-control mb-3 {{ $errors->has("fecha_fin")?"is-invalid":'' }}" name="fecha_fin" value="{{ old('fecha_fin',$catedra->fecha_fin()) }}" autocomplete="off" >
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="row">
                        <div class="col-md-6">
                            <div class="form-group">
                                <label>Cupo (*)</label>
                                <input type="number" min="1" class="form-control mb-3 {{ $errors->has("cupo")?"is-invalid":'' }}" name="cupo" value="{{ old('cupo',$catedra->cupo) }}" autocomplete="off" >
                            </div>
                        </div>
                        <div class="col-md-6">
                            <div class="form-group">
                                <label>Clases mínimas (*)</label>
                                <input type="number" min="1" name="clases_minimas" class="form-control mb-3 {{ $errors->has("clases_minimas")?"is-invalid":'' }}" value="{{ old('clases_minimas',$catedra->clases_minimas) }}" autocomplete="off">
                            </div>
                        </div>
                    </div>
                    <div class="row">
...
@section('scripts')
    ...
    <script>
        $(document).ready(function() {
            ...
            @foreach($catedra->capacitadores as $capa)

                selectedValues[{{$i}}] = {{ $capa->capacitador->id }};
                {{ $i++ }}
